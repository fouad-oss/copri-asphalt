...
- `?rf=1` → commitment pipeline portal (PIN-only login via `pipeline_user_check` RPC — `pipeline_users` has NO anon read policy, PINs stay server-side): capability-based tabs — requesters raise numbered `RF-{WO|LPO|CON}-YYYY-seq` requests (`request_submit`, idempotent via client_ref); approvers work the head-office queue (`request_decide` mints the `WO-`/`LPO-`/`CON-` commitment); everyone sees the commitment register + blanket-LPO board. Call-offs inside a live blanket's ceiling+validity auto-approve server-side. Migration 0013. **The pipeline's WO- series is COST commitments to subcontractors — a different thing from the ministry's `work_orders` table (revenue side, dispatch auto-fill); never join or mix the two.** Thresholds/approvers/rates are config rows (`approval_rules`, `pipeline_settings`, `pipeline_users`), not constants — pending the 14 July finance meeting.
