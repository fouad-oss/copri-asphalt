...
def main():
    ...
    # ── SQL ──
    head = """-- 0051_qm_expw_tadqiq — GENERATED by tools/qm_expw_tadqiq.py, do not hand-edit.
...
""" % (esc(VENDOR), esc(VENDOR))
    ...
    for r in records:
        ...
        b.append("  if v_k is not null and not exists (select 1 from qm_tadqiq where "
                 "kashef_id = v_k and vendor_id = v_vendor and serial_no = '%s'%s) then"
                 % (esc(r["serial"]),
                    " and tadqiq_date = date '%s'" % r["date"] if r["date"] else ""))
        b.append("    insert into qm_tadqiq (kashef_id, vendor_id, tadqiq_date, "
                 "street_no, note, opening, serial_no)")
        b.append("    values (v_k, v_vendor, %s, '', 'استيراد تاريخي — الطرق السريعة', "
                 "false, '%s') returning id into v_t;"
                 % ("date '%s'" % r["date"] if r["date"]
    ...
    # allocations last: allocated := executed
    ab = ["  -- ── التوزيع: الموزَّع = المنفَّذ ──"]
    for (wo, bab, band, suf), qty in sorted(alloc.items()):
