...
def emit_sql(dataset, cfg):
    # The dashboard refuses queries over ~1 MB ("Query is too large"), so the
    # backfill is packed into sequential part files, each a self-contained
    # idempotent do-block under the size cap. Paste parts in order.
    MAX_PART = 650_000

    def preamble(part_no):
        P = []
        P.append("do $qmbf$")
        P.append("declare")
        P.append("  v_contract bigint;")
        P.append("  v_copri bigint;")
        P.append("  v_k bigint;")
        P.append("  v_line bigint;")
        P.append("  v_t bigint;")
        P.append("  v_item bigint;")
        P.append("begin")
        P.append("  select id into v_contract from qm_contracts where code = 'HAW9';")
        P.append("  if v_contract is null then raise exception 'run 0033 first'; end if;")
        P.append(f"  select id into v_copri from vendors where name = '{esc(cfg['copriSelfVendorName'])}';")
        if part_no == 1:
            P.append("  if v_copri is null then")
            P.append(f"    insert into vendors (name, kind, internal, notes) values ('{esc(cfg['copriSelfVendorName'])}', 'internal', true, 'quantities module — COPRI self-performed works (backfill)') returning id into v_copri;")
            P.append("  end if;")
            P.append("  update vendors set qm_subcontractor = true where id = v_copri and not qm_subcontractor;")
            vids = sorted({s["vendorId"] for c, s in cfg["subs"].items()
                           if s.get("active", True) and s.get("vendorId")})
            if vids:
                P.append(f"  update vendors set qm_subcontractor = true where id in ({', '.join(map(str, vids))}) and not qm_subcontractor;")
        else:
            P.append("  if v_copri is null then raise exception 'paste part 1 first'; end if;")
        return P
    ...
    for key, d in sorted(dataset.items(), key=lambda kv: kv[1]["head"]["kashefNo"]):
        ...
        S.append("  end if;")
        blocks.append((h["kashefNo"], "\n".join(S)))

    # pack blocks into size-capped parts
    parts = []
    cur, cur_size, cur_range = [], 0, []
    for no, blk in blocks:
        if cur and cur_size + len(blk) > MAX_PART:
            parts.append((cur, cur_range))
            cur, cur_size, cur_range = [], 0, []
        cur.append(blk)
        cur_size += len(blk)
        cur_range.append(no)
    ...
    total = len(parts)
    for i, (blks, rng) in enumerate(parts, 1):
        head = [
            f"-- 0036_qm_backfill part {i} of {total} — GENERATED by tools/qm_backfill.py, do not hand-edit.",
            "-- Historical backfill: work orders + lines + allocations + opening tadqiq entries.",
            f"-- WOs {rng[0]}–{rng[-1]}. Paste 0035 first, then the parts IN ORDER (each is idempotent).",
        ]
        body = head + preamble(i) + blks + ["end $qmbf$;", ""]
        out = os.path.join(OUT_DIR, f"0036_qm_backfill_part{i}.sql")
        open(out, "w", encoding="utf-8").write("\n".join(body))
        print(f"SQL part {i}/{total} (WOs {rng[0]}–{rng[-1]}, {sum(len(b) for b in blks)//1000} KB) → {out}")
