...
def main():
    ...
    for wk, hes in kashefs.items():
        ...
        if kind == "wo":
            ...
            head = {"kashefNo": al["kashefNo"], "status": "kashef", "woNo": "", "woDate": None,
...
def emit_sql(dataset, cfg):
    S = []
    S.append("-- 0035_qm_backfill.sql — GENERATED by tools/qm_backfill.py, do not hand-edit.")
    S.append("-- Historical backfill: kashefs/WOs + lines + allocations + opening tadqiq entries.")
    S.append("-- Idempotent: each kashef block is skipped when (contract, kashef_no) already exists.")
    S.append("do $$")
    S.append("declare")
    ...
    for key, d in sorted(dataset.items(), key=lambda kv: kv[1]["head"]["kashefNo"]):
        ...
        S.append(
            "    insert into qm_kashefs (contract_id, kashef_no, area, loc_type, block_no, street_name, work_type, status, wo_no, wo_date, kashef_date)"
        )
        S.append(
            f"    values (v_contract, {h['kashefNo']}, '{esc(h['area'])}', '{h['locType']}', '{esc(h['blockNo'])}', "
            f"'{esc(h['streetName'])}', '{esc(h['workType'])}', '{h['status']}', '{esc(h['woNo'])}', {wo_date}, '{kdate}') returning id into v_k;"
        )
        ...
        S.append("  end if;")
    S.append("end $$;")
    out = os.path.join(OUT_DIR, "0035_qm_backfill.sql")
