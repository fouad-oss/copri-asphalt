...
**Staff-editable reference data (live-read).** Six `Ref — …` Sheet tabs (Streets, Work Orders, Sites, Drivers, Settings, Access/PINs) let non-devs add data without a deploy. Backend: `setupReferenceTabs()` creates them, `?ref=1` (`readReference_`) serves them as JSON. Client: on boot, `applyReferenceData()` overlays the cached payload onto the built-in `CONFIG` (Streets/Sites/Access are additive; Work Orders/Drivers/Settings replace when non-empty), then `runRouter()` runs; `fetchReference()` refreshes the cache in the background (stale-while-revalidate). Any failure falls back to cache → baked defaults, so the app never blocks/breaks. Staff guide: `.assets/data_reference_guide.md`. Column order matters (rows read by position). The Access tab holds PINs — keep it protected.
