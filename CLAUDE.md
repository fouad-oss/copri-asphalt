...
**Work orders = a shared picker, not auto-derived.** All three forms (dispatch, milling, materials) build their WO field via `makeWorkOrderField()`, which renders a dropdown of active contract orders filtered by the chosen site (`workOrdersFor(project, site)` over `CONFIG.contractWorkOrders`), falling back to a free-text input for projects with no list (highways / non-Copri). The list is seeded from the MPW contract-9 tracker (active/جاري orders only; see `.assets/work_orders_ref.md`). The older `CONFIG.workOrders` + `lookupWorkOrder()` single-value auto-derive is now unused by the forms — a site+block maps to several WOs by discipline, so a picker is required. Keep the separation — only ADD module code, don't entangle it with the asphalt dispatch/receipt flow. **Milling** = engineer→PM→Marco approval workflow. **Materials** = simple one-and-done record: receiver logs material (category→detailed item, qty, rate, supplier, subcontractor) + a required paper-receipt photo, which `materialSavePhoto_` decodes from base64 and stores in the "Materials Receipts" Drive folder (link written to the sheet).
